...
유연함을 얻을 수 있도록 해주는 방법에는 추상화가 있는데 그 전에 추사오하를 가능하게 해주는 다형성에 대해 
살펴보자.
<br>
<br/>
## 상속(inheritance)이란
...
예를 들어, for, while, do~while 등과 같은 문법은 반복 한다는 구문을 추상화한 것이다. 실제로는 CPU의 명령을 통해서 반복이 구현되겠지만, 이 구현으로부터 '반복'이라는 개념을 뽑아내서 for, while, do~while 등으로 추상화한 것이라고 할 수 있다.  
